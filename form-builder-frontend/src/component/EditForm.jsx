...
export default function EditForm() {
  const [formData, setFormData] = useState({ name: "", inputs: [] });
  const { id } = useParams();
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Could not edit the form");
      setFormData({
        name: "",
        inputs: [],
      });
  ...
  useEffect(() => {
    const fetchForm = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Could not fetch the form");
  ...
  return (
    <Box>
      ...
      {isFormLoading && <Loader message="Loading you Form, Please Wait!" />}
      {!isFormLoading && (
        <>
          ...
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? "Please Wait" : "Save Form"}
          </Button>
        </>
      )}
    </Box>
